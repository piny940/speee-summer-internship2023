...
class ChangeColumnsOfAssessmentForMiniulApi < ActiveRecord::Migration[7.0]
  ...
  def down
    ...
    change_column :assessments, :property_exclusive_area, :float, null: false, comment: '対象物件の専有面積'
    change_column :assessments, :property_land_area, :float, null: false, comment: '対象物件の土地面積'
    change_column :assessments, :property_building_area, :float, null: false, comment: '対象物件の建物面積'
  end
end
